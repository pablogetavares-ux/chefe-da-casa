...
import { queryOffersForAntiWaste } from "@/modules/offers/services/integrations";
...
export async function GET(request: Request) {
  try {
    ...
    const data = await queryOffersForAntiWaste(supabase, user.id, { region });

    return apiSuccess(data);
  } catch (error) {
    return handleApiRouteError(
      error,
      "GET /api/v1/offers/for-anti-waste",
      "Erro ao buscar ofertas anti-desperdício",
    );
  }
}
